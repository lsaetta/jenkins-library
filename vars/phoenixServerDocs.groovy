		...
		stages {
			...
			stage('build') {
				steps {
					...
					sh "docker build ${CACHE} --build-arg MAJOR=${PACKAGE_MAJOR} -t ${NEXUS_DOCKER_PUSH_URL}/${PROJECT}-${PACKAGE_MAJOR}-docs ."
			...
			stage('deploy') {
				steps {
					...
					sh "docker build ${CACHE} --build-arg APPNAME=${PROJECT}-${PACKAGE_MAJOR}-docs --build-arg TAGIMG=${PACKAGE_VERSION}.${BUILD_NUMBER} -t k8sctl-${PROJECT}-${PACKAGE_MAJOR} -f ./kubernetes/helm_argocd_frontend/Dockerfile ./kubernetes/helm_argocd_frontend/"
